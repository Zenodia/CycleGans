...
class CycleGAN():
    def __init__(self, lr_D=2e-4, lr_G=2e-4, image_shape=(256*1, 256*1, 3),
                 date_time_string_addition='_test', image_folder='retina'):
        ...
        self.beta_2 = 0.999
        self.batch_size = 1
        self.epochs = 2  # choose multiples of 25 since the models are save each 25th epoch
...
#         self.load_model_and_generate_synthetic_images()
        ...
        if make_predict_flag=='N':
            self.train(epochs=self.epochs, batch_size=self.batch_size, save_interval=self.save_interval)
        elif make_predict_flag=='Y':
            self.load_model_and_generate_synthetic_images()
        else:
            print("this is not a valid input , input Y or N only !")
    ...
    def train(self, epochs, batch_size=1, save_interval=1):
        ...
        for epoch in range(1, epochs + 1):
            if self.use_data_generator:
                ...
                print("len(A_train)", len(A_train), type(A_train))
...
# Help functions
    ...
    def saveImages(self, epoch, real_image_A, real_image_B, num_saved_images=1):
        directory = os.path.join('./generate_images/synthetic_images', self.date_time)
        ...
        for i in range(num_saved_images + 1):
            ...
            self.truncateAndSave(real_image_Ab, real_image_A, synthetic_image_B, reconstructed_image_A,
                                 './generate_images/synthetic_images/{}/{}/epoch{}_sample{}.png'.format(
    ...
    def save_tmp_images(self, real_image_A, real_image_B, synthetic_image_A, synthetic_image_B):
        try:
            ...
            self.truncateAndSave(None, real_images, synthetic_images, reconstructed_images,
                                 './generate_images/synthetic_images/{}/{}.png'.format(
                                     self.date_time, 'tmp'))
...
# Save and load
    ...
    def saveModel(self, model, epoch):
        ...
        directory = os.path.join('./generate_images/models/', self.date_time)
        if not os.path.exists(directory):
            os.makedirs(directory)

        model_path_w = '/generate_images/models/{}/{}_weights_epoch_{}.hdf5'.format(self.date_time, model.name, epoch)
        model.save_weights(model_path_w)
        model_path_m = '/generate_images/models/{}/{}_model_epoch_{}.json'.format(self.date_time, model.name, epoch)
        model.save_weights(model_path_m)
        json_string = model.to_json()
        with open(model_path_m, 'w') as outfile:
            json.dump(json_string, outfile)
        print('{} has been saved in /generate_images/models/{}/'.format(model.name, self.date_time))
    ...
    def load_model_and_generate_synthetic_images(self):
        ...
        if response == 'y':
            ...
            for i in range(len(synthetic_images_A)):
                ...
                print("in save_image self.testB_image_names[i].strip('png') ", self.testB_image_names[i].strip('.png'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--inference', help = "making prediction Y/N, No means you are doing training", required = True,default='N')
    args = parser.parse_args()
    make_predict_flag=args.inference
    print("making_prediction yes/no ", make_predict_flag)
    GAN = CycleGAN()
